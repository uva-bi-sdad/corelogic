...
for(j in 1:3){
  ...
  for(i in 1:nrep){
    ...
    xmat <- cbind(1, apply(coords,1, function(x){
      if(x[2]^3/20^3<x[1]/20) return(0)
      else return(1)
    }))
    
    ir4 <- apply(coords,1, function(x){
      if(x[2]^3/20^3<x[1]/20) return(0)
      else return(1)
    })
    y <- rnorm(N,intercept + alpha*ir4 + delta*(sin(std_coords[,1])+cos(std_coords[,2])),sigma)
    
    # Ordinary Least squares model
    data_frame <- data.frame(cbind(y,ir4))
    ...
    y_sp_fitted <- coef["beta0","median"]+coef["alpha","median"]*ir4+apply(post_mcmc$post_z,2,median)
...
save(sim_res, file="~/data/sim_data_4.RData")

spPlot(length = 11,col_text = "Spectral",data_frame = cbind(coords,y),shape = NULL,xlim = c(0,20), ylim=c(0,20))
spPlot(length = 11,col_text = "Spectral",data_frame = cbind(coords,fitted(model)),shape = NULL,xlim = c(0,20), ylim=c(0,20))
points(coords[ir4==1,], col="green")
points(coords[ir4==0,], col="blue")
lines(cbind(seq(0,20,length.out=1e2)^3/400,seq(0,20,length.out=1e2)), lwd=3,lty="dashed")
