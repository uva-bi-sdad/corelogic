...
for(j in 1:3){
  ...
  for(i in 1:nrep){
    ...
    # Forcing Variable setting: pattern 1
    xmat <- cbind(1,apply(coords,1 , function(x){
      if((x[1]-10)^2+(x[2]-10)^2<6^2) return(0)
      else return(1)
    }))
    
    ir5 <- apply(coords,1 , function(x){
      if((x[1]-10)^2+(x[2]-10)^2<6^2) return(0)
      else return(1)
    })
    
    y <- intercept  + rnorm(N,0,sigma)
    # + alpha*ir5+ delta*(sin(std_coords[,1])+cos(std_coords[,2]))+
    # Ordinary Least squares model
    data_frame <- data.frame(cbind(y,ir5))
    ...
    y_sp_fitted <- coef["beta0","median"]+coef["alpha","median"]*ir5+apply(post_mcmc$post_z,2,median)
...
save(sim_res, file="~/data/sim_data_5.RData")

spPlot(length = 11,col_text = "Spectral",data_frame = cbind(coords,y),shape = NULL,xlim = c(0,20), ylim=c(0,20), contour.plot = T)
spPlot(length = 11,col_text = "Spectral",data_frame = cbind(coords,fitted(model)),shape = NULL,xlim = c(0,20), ylim=c(0,20))
points(coords[ir5==1,], col="green")
points(coords[ir5==0,], col="blue")
plotrix::draw.circle(10,10,3,lty = "dashed",lwd = 3)
...
plot3D::persp3D(z=apply((matrix(surf1$z,nr=300,nc=300,byrow=T)),2,diff), contour=T, alpha=0.7, phi=40,expand=0.3, image=T)
plot3D::persp3D(z=apply((matrix(surf$z,nr=300,nc=300,byrow=T)),2,diff), contour=T, alpha=0.7, phi=40,expand=0.3, image=T)
dev.off()
