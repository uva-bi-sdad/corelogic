...
table_name <- "current_tax_2020_06_27_raw"
```
...
tbl <- fread(first_split_file, colClasses = "character")
```
...
cl <- makeForkCluster(core_num, outfile = "src/parlog")
doParallel::registerDoParallel(cl)

num_files <- length(file_paths)

res <- foreach(i = 2:num_files) %dopar% {
  con <- get_db_conn(db_host = "localhost", db_port = 5434)
  ...
  db_res <- dbWriteTable(con, c(schema_name, table_name), dt, append = TRUE, row.names = FALSE)
